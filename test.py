...
def split_att(zs, a):
    ...
    for i in range(a.size(1)):
        ...
        att_mean = temp.reshape(32, 64, 128, 128).mean(1)
        segmap = att_mean.view(32, 1, 128, 128).repeat(1, 3, 1, 1)
        #segmap = F.interpolate(att_mean, size=[128,128], mode='nearest')
        box.append(torch.sign(segmap))
...
for i in range(len(attrs_default)):
    img_a, att_a = next(dataset.gen(2, False))
    img_b, att_b = next(dataset.gen(2, True))
    ...
    if 3 == 3:
        with torch.no_grad():
            for j in range(3):
                ...
                zs_b, _ = attgan.G(img_b, mode='enc')
                #re = split_att(zs_b, att_b)
                re1, re2, re3, re4 = torch.split(zs_b, 3, dim=1)
                viz_images = torch.stack([re3,re3], dim=1)
                viz_images = viz_images.view(-1, *list(re3.size())[1:])
